...
void bindScriptCommands(ScriptCommandContext& context, const EditorAnimationScriptBindings& bindings)
{
    context.setCurrentFrame = [bindings](int frame) {
        bindings.applyAnimationState(setCurrentFrame(bindings.animationState(), frame));
    };
    context.setKeyframe = [bindings](const QString& objectName, int frame) {
        ...
        EditorAnimationState state = bindings.animationState();
        if (frame >= 0) {
            state = setCurrentFrame(state, frame);
            bindings.applyAnimationState(state);
        }

        const SceneMutationResult result = setKeyframe(bindings.sceneSnapshot(), objectId, state.currentFrame);
        if (!result.success) {
            return false;
        }

        bindings.applySceneMutation(result.scene, objectId, result.currentFrame);
        return true;
    };
    context.deleteKeyframe = [bindings](const QString& objectName, int frame) {
        ...
        const SceneMutationResult result = deleteKeyframe(bindings.sceneSnapshot(), objectId, state.currentFrame);
        if (!result.success) {
            return false;
        }
    ...
    context.copyKeyframe = [bindings](const QString& objectName, int sourceFrame, int targetFrame) {
        ...
        const SceneMutationResult result = duplicateKeyframe(bindings.sceneSnapshot(), objectId, sourceFrame, targetFrame);
        if (!result.success) {
            return false;
        }
    ...
    context.shiftKeyframes = [bindings](const QString& objectName, int frameDelta) {
        ...
        const SceneMutationResult result = shiftKeyframes(bindings.sceneSnapshot(), objectId, frameDelta);
        if (!result.success) {
            return false;
        }
    ...
    context.setAutoKey = [bindings](bool enabled) {
        bindings.applyAnimationState(setAutoKeyEnabled(bindings.animationState(), enabled));
    };
    context.setPlaybackRange = [bindings](int startFrame, int endFrame) {
        bindings.applyAnimationState(setPlaybackRange(bindings.animationState(), startFrame, endFrame));
    };
    context.setPlaybackState = [bindings](bool playing) {
        bindings.applyAnimationState(setPlaybackState(bindings.animationState(), playing));
    };
}
