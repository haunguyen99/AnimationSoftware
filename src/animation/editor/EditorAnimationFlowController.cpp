...
OperationResult setVisibleFrameRange(const EditorAnimationState& state, int startFrame, int endFrame, bool logToScript)
{
    OperationResult result;
    result.success = true;
    result.state = EditorAnimationController::setVisibleFrameRange(state, startFrame, endFrame);
    result.statusMessage = QString("Visible range: %1 to %2")
        .arg(result.state.visibleStartFrame)
        .arg(result.state.visibleEndFrame);
    if (logToScript) {
        result.commentLine = QString("visible range %1 to %2")
            .arg(result.state.visibleStartFrame)
            .arg(result.state.visibleEndFrame);
    }
    return result;
...
OperationResult setFramesPerSecond(const Context& context, const EditorAnimationState& state, int framesPerSecond, bool logToScript)
{
    const int clampedFramesPerSecond = qBound(1, framesPerSecond, 240);
    if (clampedFramesPerSecond == state.framesPerSecond) {
        OperationResult result;
        result.success = true;
        result.state = state;
        result.statusMessage = QString("Playback fps: %1").arg(state.framesPerSecond);
        return result;
    }

    const double scaleFactor = static_cast<double>(clampedFramesPerSecond) / static_cast<double>(state.framesPerSecond);
    const EditorAnimationSceneMutation::Result mutation =
        EditorAnimationSceneMutation::retimeSceneKeyframes(context.sceneSnapshot(), scaleFactor);
    if (!mutation.success) {
        return invalidSelectionResult(mutation.errorMessage, state);
    }
    ...
    result.state = EditorAnimationController::setFramesPerSecond(state, clampedFramesPerSecond);
...
OperationResult setKeyForSelection(const Context& context, const EditorAnimationState& state, std::uint64_t selectedObjectId, bool logToScript)
{
    ...
    const EditorAnimationSceneMutation::Result mutation =
        EditorAnimationSceneMutation::setKeyframe(context.sceneSnapshot(), selectedObjectId, state.currentFrame);
    if (!mutation.success) {
...
OperationResult deleteKeyForSelection(const Context& context, const EditorAnimationState& state, std::uint64_t selectedObjectId, bool logToScript)
{
    ...
    const EditorAnimationSceneMutation::Result mutation =
        EditorAnimationSceneMutation::deleteKeyframe(context.sceneSnapshot(), selectedObjectId, state.currentFrame);
    if (!mutation.success) {
    ...
    bool logToScript)
{
    ...
    const EditorAnimationSceneMutation::Result mutation =
        EditorAnimationSceneMutation::duplicateKeyframe(context.sceneSnapshot(), selectedObjectId, sourceFrame, targetFrame);
    if (!mutation.success) {
...
OperationResult shiftSelectedObjectKeyframes(const Context& context, const EditorAnimationState& state, std::uint64_t selectedObjectId, int frameDelta, bool logToScript)
{
    ...
    const EditorAnimationSceneMutation::Result mutation =
        EditorAnimationSceneMutation::shiftKeyframes(context.sceneSnapshot(), selectedObjectId, frameDelta);
    if (!mutation.success) {
...
OperationResult setPlaybackRange(const EditorAnimationState& state, int startFrame, int endFrame, bool logToScript)
{
    ...
    result.statusMessage = QString("Playback range: %1 to %2")
        .arg(result.state.playbackStartFrame)
        .arg(result.state.playbackEndFrame);
    if (logToScript) {
...
OperationResult jumpToSelectedObjectKeyframe(const Context& context, const EditorAnimationState& state, std::uint64_t selectedObjectId, bool forward, bool logToScript)
{
    ...
    const int targetFrame = EditorAnimationSceneMutation::jumpToKeyframe(
